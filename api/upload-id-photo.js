...
export default async function handler(req, res) {
  ...
  const fileName = body.fileName || 'id-photo';
  const reservationId = body.reservationId || 'unknown';
  ...
  const filePath = `reservations/${reservationId}/${side}-${Date.now()}-${crypto.randomUUID()}.${ext}`;
